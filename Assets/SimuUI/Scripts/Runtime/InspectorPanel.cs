...
namespace Assets.Scripts.SimuUI
{
    ...
    public class InspectorPanel : PanelBase<InspectorPanel>, ISimuPanel
    {
        private ElementAttbutes elementAttbutes;
        public void SetAttbutes(ElementAttbutes attbutes)
        {
        ...
        {
            SetPanelActive(true);
            for (int i = 0; i < attGameObjects.Length; i++)
            {
                bool isShowAtt = elementAttbutes.attributes.Contains(i);
                attGameObjects[i].SetActive(isShowAtt);
                if(isShowAtt) actions[i].Invoke();
            }
